  ...
  methods: {
    ...
    save() {
      const deliveryId=this.editedItem.deliveryId;
        axios.put(urls().orders+"/delivery", {
            sellerId:this.auth.userId,
            orderId:this.editedItem.orderId,
            deliveryId:this.editedItem.deliveryId,   
        }).then((response) => {
            console.log(response.data);
            const tempId = this.deliveryUsers.findIndex(item=>item.userId==deliveryId)
            this.allOrders[this.editedIndex].deliveryName=this.deliveryUsers[tempId].name; 
      });
        this.close();
    },
